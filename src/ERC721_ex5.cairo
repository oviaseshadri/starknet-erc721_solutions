...
@storage_var
func _is_breeder_approved(account : felt) -> (is_approved : felt) {
}
...
func get_is_breeder{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (is_true: felt) = _is_breeder_approved.read(address);
    return (is_true,);
}
...
func is_breeder{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(account : felt) -> (is_approved : felt) {
    ...
    let (is_approved : felt) = _is_breeder_approved.read(account);
    return (is_approved,);
}
...
func register_me_as_breeder{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (is_added : felt) {
    ...
    _is_breeder_approved.write(account=sender_address, value=1);
    return (is_added=1);
}

@external
func unregister_me_as_breeder{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (is_added : felt) {
    let (sender_address) = get_caller_address();
    _is_breeder_approved.write(account=sender_address, value=0);
    return (is_added=0);
}
...
func assert_only_breeder{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() {
    let (sender_address) = get_caller_address();
    let (is_true) = _is_breeder_approved.read(sender_address);
